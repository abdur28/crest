 ...
 *
 * Adapted from the Sapphire project's per-app audio controller.
 *
...
/// Stores per-application audio adjustments (volume, mute, 10-band EQ, and the
/// device scope an app's EQ applies to). Changes notify ``MultiAudioManager`` so
/// active taps update live.
@MainActor
final class PerAppAudioController {
    static let shared = PerAppAudioController()

    private let volumeDefaultsKey = "AtollPerAppVolumeMap"
    private let muteDefaultsKey = "AtollPerAppMuteMap"
    private let eqDefaultsKey = "AtollPerAppEQMap"
    private let eqDeviceScopeDefaultsKey = "AtollPerAppEQDeviceScopeMap"
