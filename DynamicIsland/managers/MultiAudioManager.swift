 ...
 *
 * Adapted from the Sapphire project's multi-output audio engine.
 *
...
class MultiAudioManager: ObservableObject {
    ...
    private let deviceSettingsDefaultsKey = "AtollDeviceAudioSettingsByUID"
    private let selectedOutputUIDsDefaultsKey = "AtollSelectedOutputDeviceUIDs"
    ...
    /// Whether per-app audio capture (Screen Recording) permission is granted.
    /// Multi-output taps cannot start without it.
    var hasCapturePermission: Bool {
        CGPreflightScreenCaptureAccess()
    }

    /// Prompts for Screen Recording permission (used to capture per-app audio),
    /// then starts the process monitor if it was granted.
    ...
    private func getResponsibleAppBundleID(for pid: pid_t, runningApps: [pid_t: RunningAppInfo]) -> String? {
    ...
    private func reconcileRunningApps() {
        ...
        let runningAppsByPID = MultiAudioRunningApps.infoByPID()
...
class AppTapController {
    ...
    func activate() throws {
        let queue = DispatchQueue(label: "com.atoll.audiotap.\(bundleID)", qos: .userInteractive)
...
enum CrashGuard {
    ...
    /// Note: unlike Sapphire, we deliberately do NOT hijack SIGABRT/SIGSEGV —
    /// that would interfere with Atoll's crash reporting / Sparkle. We still
    /// track aggregate devices so they can be torn down on normal invalidation.
    static func install() {}
